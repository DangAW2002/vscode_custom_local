...
export class ChatModelsWidget extends Disposable {
	...
	private create(container: HTMLElement): void {
		...
		this._register(this.addButton.onDidClick((e) => {
			...
			console.log('Dropdown actions length:', this.dropdownActions.length);
	...
	private updateAddModelsButton(): void {
		...
		// BYPASS: Always allow adding models

		// BYPASS: Always enable button
		...
		/*
		// Fallback/Legacy Copilot item (keep it if list is still empty)
		if (this.dropdownActions.length === 0) {
			this.dropdownActions.push(toAction({
				id: 'force-add-copilot',
				label: 'Connect to GitHub Copilot (Force)',
				run: async () => {
					await this.commandService.executeCommand('workbench.action.chat.triggerSetup');
				}
			}));
		}
		*/
